...
mod test {
    ...
    /// Possible scenarios:
    ///
    /// 1. A race condition among different parts within `mnx-schema.json` file?
    /// 2. A race condition within [serde_json] itself?
    ...
    fn test_tuplets_as_is_2025_10_25() {
        ...
        assert_eq!(1, parts.len());
        let Part { measures: Some(PartMeasures(part_measures)), .. } = &parts[0] else { panic!() };
